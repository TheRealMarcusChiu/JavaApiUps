...
public class JaxbShipConfirmClient {
    ...
    static {
    	...
    	try{
			...
			props.load(new FileInputStream("./" + System.getProperty("user.name") + "_build.properties"));
    	}catch (Exception e) {
	...
	private static void populateShipConfirmRequest(ShipmentConfirmRequest shipRequest) {
		...
		shipper.setShipperNumber(props.getProperty(ACCOUNT_NUMBER));
		...
		billShipper.setAccountNumber(props.getProperty(ACCOUNT_NUMBER));
